...
def user_insert_finereceipt():
    try:
        if admin_login_session() == "user":
            ...
            citizendetails_vo.citizen_id = citizen_id

            citizendetails_vo_list = citizendetails_dao.view_license_number(citizendetails_vo)
            print("citizendetails_vo_list>>>>", citizendetails_vo_list)
            citizen_license_number = citizendetails_vo_list.citizen_license_number
            print("citizen_license_number>>>>>>>>>>>>> ", citizen_license_number)
            ...
            body = 'Your Fine Details Described Below : \n\n\t Citizen Username : {} \n\t Citizen Contact : {} \n\t Citizen Driving license : {} \n\t Citizen Fine Reason : {} \n\t Citizen Fine Amount : {}'.format(
                citizen_username, citizen_contact, citizen_license_number, citizen_fine_reason, citizen_fine_amount)

            msg.attach(MIMEText(body, 'plain'))
            server = smtplib.SMTP('smtp.gmail.com', 587)
            server.starttls()
            server.login(sender, "Project202021@AiBasedEMemo")
            text = msg.as_string()
            server.sendmail(sender, receiver, text)
            server.quit()
            return redirect(url_for('user_view_fine_receipt'))
        else:
            return admin_logout_session()
    except Exception as ex:
