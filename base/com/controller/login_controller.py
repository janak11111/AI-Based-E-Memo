    ...
    try:
        ...
        if int(later - now) < 60:
            ...
            if user_otp == session_otp:
                ...
                flash(error_message, category='resend')
                return redirect(url_for('admin_load_send_otp'))
        else:
            error_message = 'Session Timed Out ! Click on Resend OTP'
            flash(error_message, category='resend')
            return redirect(url_for('admin_load_send_otp'))
...
@app.route('/admin/load_send_otp')
def admin_load_send_otp():
    try:
        return render_template('admin/loadSendOtp.html')
    except Exception as ex:
...
def admin_resend_otp():
    try:
        generate_otp()
        error_message = "OTP Sent Succesfully"
        flash(error_message, category='success')
        return render_template('admin/loadSendOtp.html')
    except Exception as ex:
